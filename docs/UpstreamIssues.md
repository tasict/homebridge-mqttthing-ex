...
| F5 | #605 #201 | `history: true` with multiple same-type services in a custom accessory crashes (duplicate fakegato UUID, no subtype) | Unique subtype per history service | implemented (src/features/history.ts) |
...
| F9 | #631 | StatusTampered bound as boolean but HAP format is UINT8 | Emit 0/1 integers (still accept truthy MQTT values) | implemented (src/services/shared.ts) |
...
| F13 | #403 #677 #366 | Missing `topics` object crashes; typo'd/case-wrong topic keys silently ignored | Config validation pass: friendly error for missing required topics; near-miss warnings | implemented for platform devices (logged at start-up) and the settings UI (src/model/validate.ts); accessory mode unchanged |
...
- Platform mode (broker defaults for all devices, one MQTT connection per broker): #493

## Popular opt-in feature requests (backlog, by demand)

1. #258 MQTT autodiscovery (HA convention)
2. #245 #342 #381 State persistence across restarts
3. #580 #623 `lockAfterms` auto-relock; #636 `turnOnAfterms`
4. #386 Thermostat separate heat/cool setpoint topics (Auto mode)
5. #397 `invertPosition` for window coverings
6. #348 #395 Accessory information via MQTT topics
7. #370 #471 #170 apply() ergonomics (log access, init hook, applyTopic)
8. #343 Read-only accessories; #346 `minStep`
9. #673 Multi-topic publish per property
10. #565 Battery topics on all types
11. #398 #460 RGB/CCT channel improvements
12. #523 FilterMaintenance; #277 Valve WaterLevel; #353 per-event topics; #435 startPub delay
