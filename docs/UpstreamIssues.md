...
|---|---|---|---|---|
| F1 | #567 #552 #617 #686 | Spurious color/state publishes on startup: adaptive-lighting internal color temperature (initialValue 140) recomputes hue/saturation and publishes uninitialized defaults to set topics during service construction | Never publish to set topics during construction; publishes only from HomeKit actions or post-priming adaptive-lighting ticks | planned (lightbulb port) |
| F2 | #431 #436 | Adaptive lighting publishes color while the light is off (turns devices on); `adaptiveLighting: "false"` (string) not honored | Suppress CT/HSV publishes while off; coerce string booleans | planned (lightbulb port) |
...
| F4 | #500 (#607) | Wildcard subscriptions (`+`/`#`) never dispatch (exact-key lookup) | MQTT topic-filter matching in dispatch | planned |
...
| F7 | #438 #458 | Null payloads crash (`message.toString()` unguarded); apply() null/undefined suppression undocumented | Null-safe decode everywhere; document "return undefined/null to ignore" | planned |
...
| F12 | #606 #670 | Empty URL passes '' to mqtt.connect; AggregateError (IPv6+IPv4 refusal) logged verbatim | Default `mqtt://localhost:1883`; unwrap AggregateError causes in logs | planned |
